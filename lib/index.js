...
const { Base, Crud, Layers, Permissions, SchemaBuilder, Schemas, Tables, JsonValidator } = services;
const utils = require('./utils');
const { errors, redis, objects } = utils;
const { setLock } = redis;
const { warn } = console;
...
class Layerize {
    ...
    constructor ({ schemas = '', keywords = '', formats = '', cacheExpireSeconds = 60 * 60 * 24 * 7, realTimeTransactions = false, layerizeSchemaName = 'layerize', coerceTypes = false } = {}) {
        ...
        this.permissions = {
            ...
            definitionGlob: '',
            definitions: {},
            validator: new JsonValidator({ coerceTypes: this.coerceTypes }).validator,
            schemas: {},
            connector: null,
            getRole: null
        };
     ...
     */
    async initiate ({ db = {}, cache = {}, es = {}, updateAll = false, install = false, schemaName = 'public', skipCoreInstall = false, permissions = {} } = {}) {
        ...
        try {
            ...
            if (this.permissions.definitionGlob !== '') {
                ...
                this.permissions.definitions = await permissions.loadDefinitions(this.permissions.definitionGlob);
        ...
        try {
            ...
            for (let i = 0; i < arySchemas.length; i++) {
                ...
                if (schema.hash !== compareSchema.hash) {
                    ...
                    warn(`WARNING: Schema '${schema.name}' needs to be updated`, schema.hash, this.schemas.hash);
