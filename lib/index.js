...
const { layerizeConfigSchemas } = require('./data/schemas');
...
pgTypes.setTypeParser(INT8_OID, (value) => parseInt(value, 10));
...
class Layerize {
     ...
     */
    constructor ({ schemas = [], keywords = '', formats = '', cacheExpireSeconds = 60 * 60 * 24 * 7, realTimeTransactions = false, layerizeSchemaName = 'layerize', layerizeEncryptionKey = '', coerceTypes = false, removeAdditionalProperties = true, auditLogs = true, app = null } = {}) {
        ...
        this.events = {
            presetEventOutputs: null,
            setEventOutputs: null,
            releaseEvents: null
        };
        this.watchers = {};
     ...
     * @param {object} [config.events={}] - events settings object.
     ...
     */
    async initiate ({ db = {}, cache = {}, es = {}, dynamo = {}, updateAll = false, install = false, schemaName = 'public', skipCoreInstall = false, permissions = {}, events = {}, watchers = [] } = {}) {
        ...
        try {
            ...
            if (Object.keys(events).length > 0) {
                ...
                this.events.presetEventOutputs = events.presetEventOutputs || null;
                this.events.setEventOutputs = events.setEventOutputs || null;
                this.events.releaseEvents = events.releaseEvents || null;

            }

            if (Object.keys(permissions).length > 0) {

                this.permissions.files = permissions.files || [];
            ...
            this.systemSchemas = await systemSchemas.load(layerizeConfigSchemas);

            /**
             * load application table schemas
             * */
            let schemas = new SchemaBuilder({ validator: this.validator });
            this.schemas = await schemas.load(this.schemaFiles);
            ...
            /**
             * load permission files if provided
             * */
            if (this.permissions.files.length > 0) {

                const permissions = new Permissions({ layerize: this });
                this.permissions.service = permissions;
                const loadDefinitions = await this.permissions.service.loadDefinitions(this.permissions.files);
                this.permissions.definitions = loadDefinitions.definitions;
                this.permissions.tableToResourceLookup = loadDefinitions.tableToResourceLookup;
    ...
    async _buildTables ({ schemaName = 'public', schemas = this.schemas, layerizeCore = false } = {}) {
        ...
        let unlock;
        let transaction = { rollback: () => true };
        let systemTransaction = { rollback: () => true };
            ...
            if (runSchema) {
                ...
                if (updateTables) {
                    ...
                    if (schemaRecord.version === schemas.version) {
                        ...
                        throw new Error(`Schema version number needs to be incremented for changes to apply. Schema '${schemaName}' current version is ${schemaRecord.version}.`);
     ...
     * @returns {Promise<object>} returns an object
     */
    __alterTableSQL ({ schemaName = '', currentTable = {}, table = {}, foreignKeys = {} } = {}) {
            ...
            let sql = this.db.schema.withSchema(schemaName).alterTable(table.name, (tbl) => {
                ...
                for (let i = 0; i < columns.length; i++) {
                    ...
                    if (typeof currentColumns[column.name] === 'undefined' || currentColumns[column.name].hash !== column.hash) {
                        ...
                        ({ foreignKeys, primaryKeys } = this.__defineTableColumn({ column, tbl, table, foreignKeys, primaryKeys, alter: (typeof currentColumns[column.name] !== 'undefined') }));
     ...
     */
    __applyForeignKeys ({ schemaName = '', tableName = '', foreignKeys = {} } = {}) {
     ...
     * @returns {Promise<object>} returns an object
     */
    __createTableSQL ({ schemaName = '', table = {}, foreignKeys = {} } = {}) {
        ...
        try {
            ...
            let sql = this.db.schema.withSchema(schemaName).createTable(table.name, (tbl) => {
                ...
                for (let i = 0; i < columns.length; i++) {
                    ...
                    ({ foreignKeys, primaryKeys } = this.__defineTableColumn({ column, tbl, table, foreignKeys, primaryKeys }));
     ...
     * @returns {Promise<object>} returns an object
     */
    __defineTableColumn ({ column = {}, tbl, table = {}, alter = false, drop = false, foreignKeys = {}, primaryKeys = [] } = {}) {
        ...
        try {
            ...
            if (column.serial) {
                ...
                ctbl = tbl.specificType(column.name, 'bigserial');
                ...
                ctbl = tbl[type](column.name, column.length);
            ...
            return { ctbl, foreignKeys, primaryKeys };
