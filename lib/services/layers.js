...
const { errors, requestFields, layerizeSchema, redis, objects, uuid, helpers, crypto } = require('../utils');
const Permissions = require('./permissions');
const _ = require('lodash');
const { removeAdditionalProperties } = layerizeSchema;
const { pick } = _;
const { setLock } = redis;
const { isUndefinedEmptyNull } = helpers;
const { error } = console;
...
class Layers {
     ...
     */
    constructor ({ layerize, name, database, cache, search, dynamoTables, schemas, validator, cacheExpireSeconds = 60 * 60 * 24 * 7, lockTimeoutMilliseconds = 90 * 1000, realTimeTransactions = false, isTransaction = false, permissions = {}, user_id = '', esWaitFor = false } = {}) {
        ...
        if (database === null) {
            ...
            throw new errors.Error({ message: 'database can not be passed as null.' });
        ...
        /**
         * An instance of the permission service
         * @member {Permissions}
         * */
        ...
        /**
         * Holds the passed permission object
         * @member {object}
         * */
        this.passedPermissions = permissions || {};
        ...
        /**
         * User Id - mainly used for audit log
         * @member {string}
         * */
        this.user_id = user_id;

        /**
         * Waits For ES to Update - it will wait until changes are live in ES (does increase latency in response, use with caution)
         * @member {string}
         * */
        this.esWaitFor = esWaitFor;
    ...
    /**
     * Combines the transactions from two different layer instances
            ...
            if (this.transactions.length > 0) {
                    ...
                    if (typeof statement !== 'undefined') {
                        ...
                        if (statement.populateToES) {
                            ...
                            if (typeof statement.keys !== 'undefined') {
                                ...
                                if (keys.length === result.rows.length) {
                        ...
                        if (Array.isArray(statement.events) && statement.events.length > 0) {

                            eventsToFire = eventsToFire.concat(statement.events);
                        ...
                        if (typeof statement.table !== 'undefined' && statement.table !== '' &&
                            ...
                            const watcherTable = { table: statement.table, on: statement.type, keys };
                            //this.debug('watcherTable', watcherTable);

                            watcherTables.push(watcherTable);

                        }
                        // else if (statement.method !== 'raw') {

                        //    console.log('statement', statement);
                        //     throw new Error('statement.type is undefined');
                ...
                try {
                    ...
                    if (populateToES.length > 0) {
                        ...
                        for (let i = 0; i < populateToES.length; i++) {
                            ...
                            if (typeof obj.items === 'undefined') {
                                ...
                                obj.items = await this.getMany(obj.table, obj.keys, { ignoreActiveLockData: true });
                    ...
                    if (this.esTransaction.length > 0) {
                        ...
                        await this.es.bulk({
                            refresh: this.esWaitFor ? 'wait_for' : false,
                    ...
                    if (auditLogsToSave.length > 0) {
                        ...
                        // skipping await as it can run in the background
                        this._saveAuditLogs({ logs: auditLogsToSave });

                    }

                    if (watcherTables.length > 0) {
                    ...
                    if (eventsToFire.length > 0) {
                        ...
                        // skipping await as it can run in the background
                        this._releaseEvents({ events: eventsToFire });
     ...
     * @param {object=} options - available options
     * @param {permission} [options.deletePermission={}] - deletePermission object
     * @param {array} [options.events=[]] - events to be fired when record is deleted
     * @param {array} [options.cacheKeysToPurge=[]] - additional cache keys to purge
     ...
     */
    async delete (table = '', key = '', { deletePermission = this.deletePermission, events = [], cacheKeysToPurge = [] } = {}) {
        ...
        try {
            ...
            return await this.deleteMany(table, keys, { deletePermission, events, cacheKeysToPurge });
     ...
     * @returns {Promise<success>} If the instance of the layer IS NOT a transaction, then it will return a success object.
     */
    async deleteByFilter (table = '', filter = '', { deletePermission = this.deletePermission, events = [], cacheKeysToPurge = [] } = {}) {
        ...
        try {
            ...
            if (typeof schema === 'undefined') {
                ...
                throw new errors.Error({ message: `The table schema '${table}' does not exist in layerize.` });
            ...
            ({ statement } = await this._applyFilters(statement, { table, filter, authorization }));
            statement.delete();
            statement.returning(schema.primaryKeys);

            if (this.isTransaction) {

                let objSQL = statement.toSQL();
                objSQL.table = table;
                objSQL.primaryKeys = schema.primaryKeys;
                objSQL.removeFromES = true;
                objSQL.removeFromCache = true;
                objSQL.events = events;
                objSQL.type = 'delete';
                objSQL.cache = [ ...cacheKeysToPurge ];
                ...
                let esTransaction = [];
                let cacheKeys = [ ...cacheKeysToPurge ];
                ...
                if (events.length > 0) {
                    ...
                    await this._releaseEvents({ events });
     ...
     * @param {object=} options - available options
     * @param {permission} [options.deletePermission={}] - permission object
     * @param {array} [options.events=[]] - events to be fired when record is deleted
     * @param {array} [options.cacheKeysToPurge=[]] - additional cache keys to purge
     * @returns {Promise<objSQL>} If the instance of the layer IS a transaction, then it will return objSQL.
     * @returns {Promise<success>} If the instance of the layer IS NOT a transaction, then it will return a success object.
     */
    async deleteMany (table = '', keys = [], { deletePermission = this.deletePermission, events = [], cacheKeysToPurge = [] } = {}) {
        ...
        try {
            ...
            let statement = this.dbSchema.clone().table(table).delete();

            if (schema.primaryKeys.length === 1) {

                statement = statement.whereIn(schema.primaryKey, keys);

            } else {

                statement = statement.whereIn(schema.primaryKeys, keys.map(objKey => schema.primaryKeys.map(primaryKey => objKey[primaryKey])));
     ...
     */
    async getAuditLog (key = '', { readPermission = this.readPermission } = {}) {

        this.debug(`getAuditLog(${key})`);
        try {

            // if permssion object is passed it verifies and throws error if not authorized and returns authorization if valid
            let authorization = await this.permissions.authorized(readPermission);

            const dynTable = this.dynamoTables.table('audit-logs', this.schemaName);
            let results = await dynTable.query(key).usingIndex('createdAtIndex').descending().loadAll().exec().promise();
     ...
     */
    async getMany (table = '', keys = [], { fields = '*', includes = '', forUpdate = false, readPermission = this.readPermission, ignoreActiveLockData = false } = {}) {
        ...
        try {
            ...
            // setting flag if multiple primary keys has been set on the select table
            const multiplePrimaryKeys = Boolean(schema.primaryKeys.length > 1);
            ...
            if (multiplePrimaryKeys) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    let primaryKeysToHash = [];

                    // checking to make sure all primary keys have been given
                    for (let x = 0; x < schema.primaryKeys.length; x++) {

                        const primaryKey = schema.primaryKeys[x];

                        if (isUndefinedEmptyNull(key[primaryKey])) {

                            throw new errors.Error({ message: `The table schema '${table}' uses multiple primary keys, you must pass the key as an object with all primary keys referenced. ex: [{ primary_key_name1: 'ad0ca9ee-3c26-467f-adf3-6ed38d04dafd', primary_key_name2: '411c7f57-7173-49ee-b740-b169ac07ca1e'}]` });

                        }

                        primaryKeysToHash.push(key[primaryKey]);

                    }

                    // creating a hash of the multiple primary keys for easier handling
                    key.__hash = crypto.md5(primaryKeysToHash.join(':'));
            ...
            if (forUpdate) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (typeof this.activeLocks[`${table}:${keyHash}`] === 'undefined') {

                        this.activeLocks[`${table}:${keyHash}`] = { key: keyHash, table, data: undefined, unlock: await setLock(this.cache.key(`${this.lockKey}:${table.toUpperCase()}`, keyHash), this.lockTimeoutMilliseconds) };
                        this.debug(`Set lock on record ${JSON.stringify(key)} in table: ${table}`);
            ...
            // creates an array, same length as keys, with null as default value
            let results = Array.from(Array(keys.length)).map(o => null);
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (typeof this.activeLocks[`${table}:${keyHash}`] !== 'undefined' && typeof this.activeLocks[`${table}:${keyHash}`].data !== 'undefined' && !ignoreActiveLockData) {

                    results[i] = this.activeLocks[`${table}:${keyHash}`].data;

                } else {

                    cacheKeys.push({
                        key: this.cache.key(this.cacheKey, table, keyHash),
                        idx: i + 0
                    });
            ...
            // when updating we want master record and as we can not 100% guarantee a cache is the latest version
            if (!forUpdate) {

                // See if caching layer has records
                if (cacheKeys.length > 0) {

                    const cacheResults = await this.cache.get(cacheKeys.map(o => o.key));

                    for (let i = 0; i < cacheKeys.length; i++) {

                        let cacheKey = cacheKeys[i];
                        let cacheResult = cacheResults[i];
                        if (cacheResult !== null) {

                            results[cacheKey.idx] = cacheResult;
            ...
            if (notFoundInCache.length > 0) {
                ...
                this.debug(`Using db to pull records from table: ${table}`);

                let statement = this.dbSchema.clone().table(table);

                if (!multiplePrimaryKeys) {

                    statement = statement.whereIn(schema.primaryKey, notFoundInCache);

                } else {

                    statement = statement.whereIn(schema.primaryKeys, notFoundInCache.map(objKey => schema.primaryKeys.map(primaryKey => objKey[primaryKey])));
                ...
                if (dbResults.length === 0) {
                    ...
                    debug(notFoundInCache);

                    throw new errors.Error({ message: `Records '${JSON.stringify(notFoundInCache)}' could not be found in '${table}' table.` });
                    ...
                    for (let i = 0; i < dbResults.length; i++) {
                        ...
                        if (multiplePrimaryKeys) {
                            ...
                            let primaryKeysToHash = schema.primaryKeys.map(primaryKey => item[primaryKey]);
                            keyHash = crypto.md5(primaryKeysToHash.join(':'));

                        }

                        results[notFoundMap[keyHash]] = item;

                        // no reason to cache it if we are updating
                        if (!forUpdate) {
                    ...
                    if (cachebulk.length > 0) {
                        ...
                        await this.cache.mset(cachebulk, this.cacheExpireSeconds);
                        this.debug(`Records put into cache for table: ${table}`);
                ...
                if (stillNotFound.length > 0) {
                    ...
                    throw new errors.Error({ message: `Records '${JSON.stringify(stillNotFound)}' could not be found in '${table}' table.` });
            ...
            if (forUpdate) {
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (multiplePrimaryKeys) {
                        ...
                        let primaryKeysToHash = schema.primaryKeys.map(primaryKey => results[i][primaryKey]);
                        keyHash = crypto.md5(primaryKeysToHash.join(':'));

                    }

                    if (typeof this.activeLocks[`${table}:${keyHash}`] !== 'undefined' && this.activeLocks[`${table}:${keyHash}`].data === 'undefined') {

                        this.activeLocks[`${table}:${keyHash}`].data = { ...results[i] };
    ...
    /**
     * Gets relationships for a given table from the data layer
     * @param {string} table - name of the table
     * @param {array} keys - primary keys of the records being requests
     * @param {object=} options - available options
     * @param {array} [options.permissionRoles=[]] - permission roles
     * @param {array} [options.permissionAction=''] - permission action
     * @returns {Promise<array>} return an array of record objects
     */
    async getRelationships (table = '', { permissionRoles = [], permissionAction = '' } = {}) {

        this.debug(`getRelationships(${table})`);
        ...
        try {
            ...
            const includes = this.schemas.includes[table];
            let source = {};
            ...
            if (typeof this.permissions.tableToResourceLookup[table] !== 'undefined') {

                await this.permissions.allowed({ role: permissionRoles, action: permissionAction, resource: this.permissions.tableToResourceLookup[table] });
            ...
            throw this.error.handle({ error, caller: 'getRelationships' });
     ...
     * @param {string|array} [options.includes=''] - adds other records into return object
     * @param {permission} [options.createPermission={}] - permission object
     * @param {array} [options.events=[]] - events to be fired when record is created
     * @param {array} [options.cacheKeysToPurge=[]] - additional cache keys to purge
     * @param {AsyncFunction} [options.validation=null] - an async validation event to run
     * @returns {Promise<objSQL>} If the instance of the layer IS a transaction, then it will return objSQL.
     ...
     */
    async insert (table = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', createPermission = this.createPermission, events = [], cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            let results = await this.insertMany(table, data, { returnRecords: returnRecord, ignoreReadOnly, fields, includes, createPermission, events, cacheKeysToPurge, validation });
     ...
     * @param {string|array} [options.includes=''] - adds other records into return object
     * @param {permission} [options.createPermission={}] - createPermission object
     * @param {array} [options.events=[]] - events to be fired when record is created
     * @param {array} [options.cacheKeysToPurge=[]] - additional cache keys to purge
     * @param {AsyncFunction} [options.validation=null] - an async validation event to run
     ...
     * @throws {Error} The 'data' parameter is required when doing an insert() or insertMany().
     */
    async insertMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', createPermission = this.createPermission, events = [], cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            if (data.length === 0) {
                ...
                throw new errors.Error({ message: 'At least one record is required when doing an insert() or insertMany() to be added.' });
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                ({ data: data[i] } = await this._validate({ table, data: data[i], validation }));
            ...
            if (this.isTransaction) {
                ...
                return await this.__nonTransactionESCacheProcessing({ table, schema, statement, returnRecords, fields, includes, authorization, events, cacheKeysToPurge });
     ...
     * @param {boolean} [options.returnRecords=false] - will return new record, if not a transaction
     * @param {string|array} [options.includes=''] - adds other records into return object
     * @param {permission} [options.updatePermission={}] - updatePermission object
     * @param {array} [options.events=[]] - events to be fired when record updates
     * @param {array} [options.cacheKeysToPurge=[]] - additional cache keys to purge
     * @param {AsyncFunction} [options.validation=null] - an async validation event to run
     * @returns {Promise<objSQL>} If layer IS a transaction, then it will return objSQL
     * @returns {Promise<object>} If layer IS NOT a transaction and returnRecord is 'true', then it will return the reocrd object
     ...
     */
    async patch (table = '', key = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, events = [], cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            ({ data } = this._mergePatch({ currentRecord, patch: data, schema, ignoreReadOnly }));

            // validate against json schema and remove any fields that are readonly, unless ignored
            ({ data } = await this._validate({ table, data, validation }));
            ...
            if (authorization.conditions.length > 0 || this.auditLogs) {
                ...
                ({ item: data, saveUpdates, auditLogs } = await this.__authorizingConditions({ authorization, currentRecords: [ currentRecord ], schema, key, item: data, table, auditLogs, saveUpdates, ignoreReadOnly }));
            ...
            if (saveUpdates) {
                ...
                if (events.length > 0) {
                    ...
                    events = await this._presetEventOutputs({
                        events,
                        transaction: this.isTransaction ? this : layer
                    });
                ...
                if (this.isTransaction) {
                    ...
                    objSQL.cache = [cacheKey, ...cacheKeysToPurge];
                    ...
                    let results = await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys: [ key ], schema, statement, returnRecords: returnRecord, fields, includes, authorization, events, cacheKeysToPurge, layer });
                    return results[0];
     ...
     */
    async patchByFilter (table = '', filter = '', data = {}, { ignoreReadOnly = [], updatePermission = this.updatePermission, events = [], cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            ({ data } = await this._validate({ table, data, schema: narrowedSchema, validation }));
            ...
            if (this.isTransaction) {
                ...
                if (cacheKeysToPurge.length > 0) {

                    await this.cache.clear(...cacheKeysToPurge);
     ...
     */
    async patchMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, events = [], cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (isUndefinedEmptyNull(item[schema.primaryKey])) {
                ...
                ({ data: item } = await this._validate({ table, data: item, schema: schemaMap[hash], validation }));

                // TODO: MISSING AUDIT LOG
            ...
            if (this.isTransaction) {
                ...
                statements[0].cache = statements[0].cache.concat(cacheKeysToPurge);
                ...
                return await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys, schema, statement: statements, returnRecords, fields, includes, authorization, events, cacheKeysToPurge });
    ...
    async search (table = '', {
        ...
        try {
            ...
            if (schema.esEnabled && (type === 'auto' || type === 'es') && joins.length === 0) {
                ...
                let joinFields = [];
                ({ statement, joinFields } = this._applyJoins(statement, { table, joins }));
                ...
                total = parseInt((await countStatement)[0].count, 10);
     ...
     */
    transaction ({ realTime = this.realTimeTransactions, esWaitFor = this.esWaitFor } = {}) {
        ...
        try {
            ...
            return new Layers({
                ...
                permissions: this.passedPermissions,
                esWaitFor: esWaitFor
            });
     ...
     * @returns {Promise<success>} If layer IS NOT a transaction and returnRecord is 'false', then it will return a success object
     */
    async update (table = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, events = [], cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            if (saveUpdates) {
                ...
                if (this.isTransaction) {
                    ...
                    let results = await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys: [ key ], schema, statement, returnRecords: returnRecord, fields, includes, authorization, auditLogs, events, cacheKeysToPurge, layer });
     ...
     * @returns {Promise<success>} If layer IS NOT a transaction and returnRecord is 'false', then it will return a success object
     */
    async updateMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, cacheKeysToPurge = [], validation = null } = {}) {
        ...
        try {
            ...
            // looping seperately so we can throw an error if primary key is not present
            for (let i = 0; i < data.length; i++) {
                ...
                ({ data: item } = await this._validate({ table, data: item, validation }));
            ...
            for (let j = 0; j < data.length; j++) {
                ...
                if (authorization.conditions.length > 0 || this.auditLogs) {
                    ...
                    ({ item, saveUpdates, auditLogs } = await this.__authorizingConditions({ authorization, currentRecords, schema, key, item, table, auditLogs, ignoreReadOnly, saveUpdates }));
            ...
            if (this.isTransaction) {
                ...
                return await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys, schema, statement: statements, returnRecords, fields, includes, authorization, auditLogs, cacheKeysToPurge, layer });
        ...
        try {
            ...
            if (results.length > 0) {
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (result[property] !== '' && result[property] !== 'null' && result[property] !== null) {
    ...
    /**
     * A protected internal method for applying the search criteria 'joins' to a db statement.
     * @access protected
     * @param {knex} statement - the db statement
     * @param {object=} options - available options
     * @param {string} [options.table=''] - name of the table
     * @param {array} [options.filter=[]] - layerize filter syntax
     * @returns {Promise<knex>} The extended passed in statement
     * @throws {Error} A native DB filter can not be used within an ES query.
     */
    _applyJoins (statement, { table = '', joins = [] } = {}) {

        this.debug(`_applyJoins(${table})`);
        try {

            let joinFields = [ `${table}.*` ];
            ...
            for (let i = 0; i < joins.length; i++) {
                ...
                if (typeof join === 'string' || typeof join === 'object') {

                    if (join.native) {

                        statement.joinRaw(join.statement);

                    } else {

                        let joinName = join;
                        let joinType = 'leftJoin';
                        ...
                        if (typeof join === 'object') {
                            ...
                            if (typeof join.type !== 'undefined') {
                                ...
                                switch (join.type) {
                                    ...
                                    default:
                                        throw new errors.Error({ message: `Unrecongized join type: '${join.type}'.` });
                        ...
                        if (typeof this.schemas.includes[table].source[joinName] === 'undefined' && typeof this.schemas.includes[table].referenced[joinName] === 'undefined') {

                            throw new errors.Error({ message: `Unable to find join relationship between '${table}' and '${join}'.` });

                        }

                        let includeDef = this.schemas.includes[table].source[join];
                        let joinTableName;

                        if (typeof includeDef === 'undefined') {

                            includeDef = this.schemas.includes[table].referenced[join];
                            joinTableName = includeDef.propertyTable;

                        } else {

                            joinTableName = includeDef.table;

                        }

                        statement[joinType](joinTableName, (join) => {

                            join.on(`${includeDef.table}.${includeDef.column}`, '=', `${includeDef.propertyTable}.${includeDef.property}`);

                        });

                        joinFields.push(`row_to_json(${joinTableName}.*) as ${joinName}`);
            ...
            return { statement, joinFields: this.database.raw(joinFields) };

        } catch (error) {

            throw this.error.handle({ error, caller: '_applyJoins' });
    ...
    /**
     * A protected internal method for preping an event's output.
     * @access protected
     * @param {object=} options - available options
     * @param {array} [options.events=[]] - events to be fired
     * @returns {Promise<events>} events
     */
    async _presetEventOutputs ({ events = [] }) {

        this.debug('_presetEventOutputs');
        try {

            let presetEventOutputs = this.layerize.events.presetEventOutputs;
            if (typeof presetEventOutputs === 'function') {

                events = await presetEventOutputs(events, { layerize: this.layerize, schemaName: this.schemaName });
            ...
            throw this.error.handle({ error, caller: '_presetEventOutputs' });
    ...
    /**
     * A protected internal method for finalizing an event's output.
     * @access protected
     * @param {object=} options - available options
     * @param {array} [options.events=[]] - events to be fired
     * @returns {Promise<events>} events
     */
    async _setEventOutputs ({ events = [] }) {

        this.debug('_setEventOutputs');
        try {

            let setEventOutputs = this.layerize.events.setEventOutputs;
            if (typeof setEventOutputs === 'function') {

                events = await setEventOutputs(events, { layerize: this.layerize, schemaName: this.schemaName });
            ...
            throw this.error.handle({ error, caller: '_setEventOutputs' });
    ...
    /**
     * A protected internal method for removing additional properties.
     * @access protected
     * @param {object=} options - available options
     * @param {array} [options.table=''] - table name of the schema
     * @param {array} [options.data={}] - data to be cleaned
     * @returns {Promise<events>} events
     */
    _removeAdditionalProperties ({ table = '', data = {} }) {

        try {

            if (table === '') {

                throw new errors.Error({ message: 'passing a table name is required when using _removeAdditionalProperties ()' });
            ...
            // remove addition properties that are not defined
            return removeAdditionalProperties({ data, allowedProperties: schema.allowedProperties });

        } catch (error) {

            throw this.error.handle({ error, caller: '_removeAdditionalProperties' });
    ...
    /**
     * A protected internal method for releaseing events.
     * @access protected
     * @param {object=} options - available options
     * @param {array} [options.events=[]] - events to be fired
     * @returns {Promise<events>} events
     */
    async _releaseEvents ({ events = [] }) {

        this.debug('_releaseEvents');
        try {

            let releaseEvents = this.layerize.events.releaseEvents;
            if (typeof releaseEvents === 'function') {

                events = await this._setEventOutputs(events);
                events = await releaseEvents(events, { layerize: this.layerize, schemaName: this.schemaName });
            ...
            throw this.error.handle({ error, caller: '_releaseEvents' });
    ...
    /**
     * A protected internal method for releasing active locks.
     * @access protected
     * @param {object=} options - available options
     * @param {array} [options.events=[]] - events to be fired
    ...
    _releaseActiveLocks () {
        ...
        this.debug('_releaseActiveLocks');
        try {

            Object.keys(this.activeLocks).map((key) => {

                const activeLock = this.activeLocks[key];

                this.debug(`Released lock on record ${activeLock.key} in table: ${activeLock.table}`);
            ...
            throw this.error.handle({ error, caller: '_releaseActiveLocks' });
     ...
     */
    _mergePatch ({ currentRecord = {}, patch = {}, schema = {}, ignoreReadOnly = [] } = {}) {
        ...
        try {
             ...
             * @param {object} [additionalProperties=false] - allow additional properties
             * @returns {object} patched object
             */
            let crawl = (newObj = {}, patch = {}, properties = {}, additionalProperties = false) => {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (typeof patch[key] === 'object' && !Array.isArray(patch[key]) && typeof properties[key] !== 'undefined' && patch[key] !== null && Object.keys(patch[key]).length > 0) {
                        ...
                        newObj[key] = crawl(newObj[key], patch[key], properties[key].properties, properties[key].additionalProperties);

                    } else {

                        if ((typeof properties[key] === 'undefined' && additionalProperties) || (typeof properties[key] !== 'undefined' && !properties[key].readOnly)) {
            ...
            for (let i = 0; i < tables.length; i++) {
                ...
                if (typeof this.layerize.watchers[obj.table] !== 'undefined') {
                    ...
                    if (typeof this.layerize.watchers[obj.table][obj.on] !== 'undefined') {
                        ...
                        let watchers = Object.keys(this.layerize.watchers[obj.table][obj.on]);

                        for (let x = 0; x < watchers.length; x++) {

                            let watcher = watchers[x];
                            await this.layerize.watchers[obj.table][obj.on][watcher](obj.keys, { action: obj.on, layerize: this.layerize, schemaName: this.schemaName });
    ...
    _stringToObjectFilters ({ table = '', filter = '' } = {}) {
        ...
        if (!objFilter.native.active) {
            ...
            if (Array.isArray(filter)) {
                ...
                let strFilter = '';
                filter = filter.filter(obj => !isUndefinedEmptyNull(obj));
     ...
     * @param {AsyncFunction} [options.validation=null] - an async validation event to run
     * @returns {Promise<object>} returns data object
     */
    async _validate ({ table = '', data = {}, schema = {}, validation = null } = {}) {
        ...
        try {
            ...
            try {
                ...
                // must be after validate
                data = this._removeAdditionalProperties({ table, data });

                if (!isUndefinedEmptyNull(validation)) {

                    data = await validation({ data });
     ...
     */
    async __authorizingConditions ({ authorization, currentRecords, schema, key = '', item = {}, table = '', auditLogs = [], saveUpdates = true, ignoreReadOnly = [], type = 'update' }) {

        try {

            // clean item and remove fields that can not be updated
            ...
            if (authorization.conditions.length > 0 || this.auditLogs) {
                ...
                if (this.auditLogs) {
                    ...
                    let copyCurrentRecord = { ...currentRecord };
                    ...
                    for (let x = 0; x < currentRecordProperties.length; x++) {
                        ...
                        if (typeof item[propertyName] !== typeof copyCurrentRecord[propertyName] && (item[propertyName] !== null && copyCurrentRecord[propertyName] !== null)) {
                    ...
                    if (differences.length > 0) {
                        ...
                        saveUpdates = false;
                        debug('no changes found, so skipping saving');
     ...
     */
    async __nonTransactionESCacheProcessing ({ type = 'insert', table, keys = [], schema, statement, returnRecords = false, fields = '*', includes = '', authorization, auditLogs = [], events = [], cacheKeysToPurge = [], layer = this.transaction() } = {}) {
        ...
        try {
            ...
            if (auditLogs.length > 0) {
                ...
                // skipping await as it can run in the background
                this._saveAuditLogs({ logs: auditLogs });
     ...
     * @returns {Promise<object>} returns an object containing the passed in data parameter
     */
    __convertForDatabase ({ type = 'update', table = '', data = {}, schema = {}, ignoreReadOnly = [] } = {}) {
