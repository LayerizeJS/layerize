...
const { errors, requestFields, layerizeSchema, redis, objects, uuid, helpers } = require('../utils');
        ...
        try {
            ...
            if (forUpdate) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (typeof this.activeLocks[`${table}:${key}`] === 'undefined') {

                        this.activeLocks[`${table}:${key}`] = { key, table, data: undefined, unlock: await setLock(this.cache.key(`${this.lockKey}:${table.toUpperCase()}`, key), this.lockTimeoutMilliseconds) };
                        this.debug(`Set lock on record ${key} in table: ${table}`);
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                let key = keys[i];
                if (typeof this.activeLocks[`${table}:${key}`] !== 'undefined' && typeof this.activeLocks[`${table}:${key}`].data !== 'undefined' && !ignoreActiveLockData) {

                    results[i] = this.activeLocks[`${table}:${key}`].data;

                } else {

                    cacheKeys.push({
                        key: this.cache.key(this.cacheKey, table, key),
                        idx: i + 0
                    });
            ...
            if (forUpdate) {
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (typeof this.activeLocks[`${table}:${key}`] !== 'undefined' && this.activeLocks[`${table}:${key}`].data === 'undefined') {

                        this.activeLocks[`${table}:${key}`].data = { ...results[i] };
