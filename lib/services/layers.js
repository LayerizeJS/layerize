...
class Layers {
     ...
     */
    constructor ({ layerize, name, database, cache, search, dynamoTables, schemas, validator, cacheExpireSeconds = 60 * 60 * 24 * 7, lockTimeoutMilliseconds = 30 * 1000, realTimeTransactions = false, isTransaction = false, permissions = {}, user_id = '', esWaitFor = false } = {}) {
    ...
    async getMany (table = '', keys = [], { fields = '*', includes = '', forUpdate = false, readPermission = this.readPermission, ignoreActiveLockData = false } = {}) {
        ...
        try {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                let key = keys[i];
                if (typeof this.activeLocks[key] !== 'undefined' && !ignoreActiveLockData) {
    ...
    async search (table = '', {
        ...
        try {
            ...
            if (schema.esEnabled && (type === 'auto' || type === 'es') && joins.length === 0) {
                ...
                // defining table name or erro will occur if primaryKey name is not unqiue accross joined in tables
                if (sort === schema.primaryKey) {

                    sort = `${table}.${schema.primaryKey}`;
