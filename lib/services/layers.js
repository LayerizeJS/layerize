...
class Layers {
    ...
    async commit ({ transactions = [], autoRollback = true } = {}) {
        ...
        try {
            ...
            Object.keys(this.activeLocks).map((key) => {
                ...
                this.debug(`released lock on record ${key}`);
    ...
    async getMany (table = '', keys = [], { fields = '*', includes = '', forUpdate = false, readPermission = this.readPermission, ignoreActiveLockData = false } = {}) {
        ...
        try {
            ...
            if (forUpdate) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (typeof this.activeLocks[key] === 'undefined') {
                        ...
                        this.activeLocks[key] = { key, data: undefined, unlock: await setLock(this.cache.key(this.lockKey, key), this.lockTimeoutMilliseconds) };
                        this.debug(`lock set on record ${key} in table ${table}`);
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                let key = keys[i];
                if (typeof this.activeLocks[key] !== 'undefined' && typeof this.activeLocks[key].data !== 'undefined' && !ignoreActiveLockData) {
            ...
            if (forUpdate) {
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (typeof this.activeLocks[key] !== 'undefined' && this.activeLocks[key].data === 'undefined') {

                        this.activeLocks[key].data = { ...results[i] };
    ...
    _removeAdditionalProperties ({ table = '', data = {} }) {
        ...
        try {
            ...
            const allowedProperties = this.layerize.schemas.layerize[table].allowedProperties;
                    ...
                    if (data.hasOwnProperty(propertyName)) {
                        ...
                        if (typeof allowedProperties.embeddedArrays[propertyName] !== 'undefined') {
                            ...
                            for (let i = 0; i < data[propertyName].length; i++) {
                                ...
                                data[propertyName][i] = recursive(data[propertyName][i], allowedProperties.embeddedArrays[propertyName]);
