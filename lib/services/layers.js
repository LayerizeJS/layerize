...
class Layers {
     ...
     */
    constructor ({ layerize, name, database, cache, search, schemas, validator, cacheExpireSeconds = 60 * 60 * 24 * 7, lockTimeoutMilliseconds = 30 * 1000, realTimeTransactions = false, isTransaction = false } = {}) {
        ...
        /**
         * Holds the active locks that get release when transaction is committed or rolled back
         * @member {array}
         * */
        this.activeLocks = [];
    ...
    combine (layer) {
        ...
        try {
            ...
            this.activeLocks = this.activeLocks.concat(layer.activeLocks);
     ...
     */
    async count (table = '', { fields = '*', filter = '', estimated = false, permission = {} } = {}) {

        this.debug('count()');
        try {

            // if permssion object is passed it verifies and throws error if not authorized and returns authorization if valid
            let authorization = await this.permissions.authorized(permission);
     ...
     */
    async delete (table = '', key = '', { permission = {} } = {}) {
        ...
        try {
            ...
            return await this.deleteMany(table, keys, { permission });
     ...
     * @returns {Promise<success>} If the instance of the layer IS NOT a transaction, then it will return a success object.
     */
    async deleteByFilter (table = '', filter = '', { permission = {} } = {}) {
        ...
        try {
            ...
            if (typeof schema === 'undefined') {
                ...
                throw new Error(`The schema '${table} does not exist in layerize.'`);
     ...
     * @returns {Promise<success>} If the instance of the layer IS NOT a transaction, then it will return a success object.
     */
    async deleteMany (table = '', keys = [], { permission = {} } = {}) {

        this.debug('deleteMany()', permission);
        try {
     ...
     */
    async get (table = '', key = '', { fields = '*', includes = '', forUpdate = false, permission = {} } = {}) {
        ...
        try {
            ...
            let results = await this.getMany(table, keys, { fields, includes, forUpdate, permission });
     ...
     */
    async getMany (table = '', keys = [], { fields = '*', includes = '', forUpdate = false, permission = {} } = {}) {
        ...
        try {
            ...
            let response = requestFields(results, { stacked: [...authorization.properties, fields] });
     ...
     */
    async insert (table = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
        ...
        try {
            ...
            let results = await this.insertMany(table, data, { returnRecords: returnRecord, ignoreReadOnly, fields, includes, permission });
     ...
     * @throws {Error} The 'data' parameter is required when doing an insert() or insertMany().
     */
    async insertMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
        ...
        try {
            ...
            if (table === '') {
                ...
                throw new Error('The \'table\' parameter is required when doing an insert() or insertMany().');

            }

            if (data.length === 0) {

                throw new Error('Data parameter is required when doing an insert() or insertMany()');
     ...
     */
    async patch (table = '', key = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
        ...
        try {
            ...
            if (table === '') {
                ...
                throw new Error('The \'table\' parameter is required when doing an patch().');

            }

            if (table === '') {

                throw new Error('The \'key\' parameter is required when doing an patch().');
            ...
            if (Object.keys(data).length === 0) {
                ...
                throw new Error('The \'data\' parameter is required when doing an patch().');
            ...
            if (this.isTransaction) {
                ...
                let results = await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys: [ key ], schema, statement, returnRecords: returnRecord, fields, includes, authorization });
                return results[0];
     ...
     */
    async patchByFilter (table = '', filter = '', data = {}, { ignoreReadOnly = [], permission = {} } = {}) {
     ...
     */
    async patchMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
    ...
    async search (table = '', {
        ...
        try {
            ...
            if (schema.esEnabled && (type === 'auto' || type === 'es') && joins.length === 0) {
                ...
                results = requestFields(results, { stacked: [...authorization.properties, fields] });
     ...
     * @returns {Promise<success>} If layer IS NOT a transaction and returnRecord is 'false', then it will return a success object
     */
    async update (table = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
        ...
        try {
            ...
            if (typeof key === 'undefined') {
                ...
                throw new Error('When using update(), the object must contain the primary key with all other properties.');
     ...
     * @param {permission} [options.permission={}] - permission object
     * @returns {Promise<objSQL>} If layer IS a transaction, then it will return objSQL
     * @returns {Promise<array>} If layer IS NOT a transaction and returnRecord is 'true', then it will return the array of reocrd objects
     * @returns {Promise<success>} If layer IS NOT a transaction and returnRecord is 'false', then it will return a success object
     */
    async updateMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
            ...
            if (objFilter.native.active) {
                ...
                if (isEsStatement) {
                    ...
                    if (objFilter.native.type === 'db') {
                        ...
                        throw new Error('A native DB filter can not be used within an ES query.');
                    ...
                    if (objFilter.native.type === 'es') {
                        ...
                        throw new Error('A native ES filter can not be used within a DB query.');
    ...
    _esFilter (objFilter = {}, authorization = {}) {
        ...
        this.debug('_esFilter', authorization);
        try {
     ...
     */
    async __nonTransactionESCacheProcessing ({ type = 'insert', table, keys = [], schema, statement, returnRecords = false, fields = '*', includes = '', authorization } = {}) {
