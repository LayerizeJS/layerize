...
class Layers {
     ...
     */
    async delete (table = '', key = '', { deletePermission = this.deletePermission, events = [] } = {}) {
        ...
        try {
            ...
            return await this.deleteMany(table, keys, { deletePermission, events });
     ...
     * @returns {Promise<success>} If the instance of the layer IS NOT a transaction, then it will return a success object.
     */
    async deleteByFilter (table = '', filter = '', { deletePermission = this.deletePermission, events = [] } = {}) {
     ...
     * @returns {Promise<success>} If the instance of the layer IS NOT a transaction, then it will return a success object.
     */
    async deleteMany (table = '', keys = [], { deletePermission = this.deletePermission, events = [] } = {}) {
        ...
        try {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (typeof this.activeLocks[`${table}:${key}`] !== 'undefined' && typeof this.activeLocks[`${table}:${key}`].data !== 'undefined' && !ignoreActiveLockData) {
                    ...
                    fromActiveLocks.push(this.activeLocks[`${table}:${key}`].data);
     ...
     */
    async insert (table = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', createPermission = this.createPermission, events = [], validation = null } = {}) {
        ...
        try {
            ...
            let results = await this.insertMany(table, data, { returnRecords: returnRecord, ignoreReadOnly, fields, includes, createPermission, events, validation });
     ...
     * @throws {Error} The 'data' parameter is required when doing an insert() or insertMany().
     */
    async insertMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', createPermission = this.createPermission, events = [], validation = null } = {}) {
        ...
        try {
            ...
            if (this.isTransaction) {
                ...
                return await this.__nonTransactionESCacheProcessing({ table, schema, statement, returnRecords, fields, includes, authorization, events });
     ...
     */
    async patch (table = '', key = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, events = [], validation = null } = {}) {
        ...
        try {
            ...
            if (saveUpdates) {
                ...
                if (this.isTransaction) {
                    ...
                    let results = await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys: [ key ], schema, statement, returnRecords: returnRecord, fields, includes, authorization, events, layer });
                    return results[0];
     ...
     */
    async patchByFilter (table = '', filter = '', data = {}, { ignoreReadOnly = [], updatePermission = this.updatePermission, events = [], validation = null } = {}) {
     ...
     */
    async patchMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, events = [], validation = null } = {}) {
        ...
        try {
            ...
            if (this.isTransaction) {
                ...
                return await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys, schema, statement: statements, returnRecords, fields, includes, authorization, events });
     ...
     * @returns {Promise<success>} If layer IS NOT a transaction and returnRecord is 'false', then it will return a success object
     */
    async update (table = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, events = [], validation = null } = {}) {
        ...
        try {
            ...
            if (saveUpdates) {
                ...
                if (this.isTransaction) {
                    ...
                    let results = await this.__nonTransactionESCacheProcessing({ type: 'update', table, keys: [ key ], schema, statement, returnRecords: returnRecord, fields, includes, authorization, auditLogs, events, layer });
     ...
     * @returns {Promise<success>} If layer IS NOT a transaction and returnRecord is 'false', then it will return a success object
     */
    async updateMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', updatePermission = this.updatePermission, validation = null } = {}) {
     ...
     */
    async __nonTransactionESCacheProcessing ({ type = 'insert', table, keys = [], schema, statement, returnRecords = false, fields = '*', includes = '', authorization, auditLogs = [], events = [], layer = this.transaction() } = {}) {
