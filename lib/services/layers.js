...
class Layers {
     ...
     */
    constructor ({ layerize, name, database, cache, search, dynamoTables, schemas, validator, cacheExpireSeconds = 60 * 60 * 24 * 7, lockTimeoutMilliseconds = 30 * 1000, realTimeTransactions = false, isTransaction = false, permissions = {}, user_id = '' } = {}) {
     ...
     */
    async getAuditLog (table = '', key = '', { readPermission = this.readPermission } = {}) {

        this.debug('getAuditLog()');
        try {
    ...
    async patch (table = '', key = '', data = {}, { returnRecord = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
        ...
        try {
            ...
            let currentRecord = await layer.get(table, key, { forUpdate: true });
    ...
    async updateMany (table = '', data = [], { returnRecords = false, ignoreReadOnly = [], fields = '*', includes = '', permission = {} } = {}) {
        ...
        try {
            ...
            if (authorization.conditions.length > 0 || this.auditLogs) {
                ...
                currentRecords = await layer.getMany(table, keys, { forUpdate: true });
