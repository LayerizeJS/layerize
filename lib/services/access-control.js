    ...
    can ({ role = [], action = '', resource = '', context } = {}) {
        ...
        try {
            ...
            if (role.length === 0) {
                ...
                throw new Error('Internal Permission Error: At least one one user role must be provided.');
            ...
            for (let i = 0; i < role.length; i++) {
                ...
                if (typeof this.grants[rol] === 'undefined') {
                    ...
                    throw new Error(`Internal Permission Error: The provided user role '${rol}' could not be found in initialized grants.`);
            ...
            let objGrant = {
                ...
                _: aryGrants.filter(grant => grant._ !== null).map(grant => grant._)
            };

            if (aryGrants.length > 0) {

                objGrant.granted = true;
                objGrant.grant.pliminary = true;
                objGrant.grant.full = true;
    ...
    accessList ({ role = [], grants = {} } = {}) {
        ...
        try {
            ...
            if (role.length === 0) {
                ...
                throw new Error('At least one role must provided when using Permission.accessList() method');

            }

            if (Object.keys(grants).length === 0) {

                throw new Error('A grants object must provided when using Permission.accessList() method.');
