...
class Crud extends Base {
    ...
    async delete (key = '') {
        ...
        let transaction = { rollback: new Promise(success => success(true)) };
        ...
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (typeof item[schema.primaryKey] === 'undefined') {
                    ...
                    throw new this.errors.Error({ message: 'When using patchMany(), each object in the data array must contain the primary keys.' });
     ...
     */
    __additionalValidation ({ action = 'insert', key = '', data, options, returnRecord, layers } = {}) {
